...
재활 훈련 목적상 사용자의 미세한 힘 조절이

게임 내 타격력에 그대로 반영돼야 하는데, 이 상태로는 절반 이상의

힘 구간이 전부 "최대 세기"로 뭉뚱그려져서 의미가 없었습니다.



기본 API 값을 보정하는 옵션은 따로 없어서, 손가락 관절 데이터를

직접 가공해 강도를 계산하는 방향으로 접근하였습니다.
...
엄지는 나머지 손가락과 관절 구조가 달라서 따로 계산했습니다.



**엄지**: 손가락 방향 벡터와 손바닥 법선 벡터의 내적을 구하고,

`InverseLerp`로 유의미한 구간(-0.7 ~ 0.3)만 0~1로 재매핑했습니다.
...
내적으로 굽힘 정도를 구했습니다. 펴져 있으면 두 벡터가 거의 같은 방향(내적≈1),

굽으면 벌어지는(내적↓) 원리를 이용했습니다.
...
3차원 거리를 재서 정확도를 계산합니다. 0.05m 이내면 100%,

0.30m 이상 벗어나면 0%로 처리했습니다.
...
가중치로 곱해서 최종 타격력을 정합니다. 정확하게 칠수록 더 큰

힘이 들어가게 만들어서, 정밀한 동작을 자연스럽게 유도하고 싶었습니다.
...
전달되어, 힘이 클수록 파괴 반경과 제거되는 조각 수가 늘어나게 했습니다.
